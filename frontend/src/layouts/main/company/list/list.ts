...
export default {
  ...
  data() {
    return {
      loading: false,
      headers: [
        { title: 'Recnum', value: 'recnum', align: 'center' },
        { title: 'Código', value: 'codigo', align: 'center' },
        { title: 'Empresa', value: 'empresa', align: 'center' },
        { title: 'Sigla', value: 'sigla', align: 'center' },
  ...
  methods: {
    ...
    edit(item) {
      this.$router.push('/company/record/' + item.codigo);
    },
    destroy(item) {
      new Swal({
        icon:"warning",
        ...
        if (result.isConfirmed) {
          try {
            await CompanyService.build().delete(item.codigo);
            await this.load();
          } catch (error) {
            new Swal({
              icon:"error",
